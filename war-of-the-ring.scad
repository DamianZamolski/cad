...
cards_holder = [
  card.x + cards_padding + 2 * t, card.y + cards_padding + 2 * t, insert.z
];
corner_width = cards_holder.x * 0.25;
finger_hole_radius = (cards_holder.x - 2 * corner_width) / 2;
...
translate([ 2 * (double_nation.x + gap), 0, 0 ]) {
  ...
  translate([ cards_holder.y, tokens.y + gap, 0 ]) rotate([ 0, 0, 90 ])
      Multi_Holder(size = cards_holder, corner_width = corner_width,
                   thickness = t, x_finger_hole_radius = finger_hole_radius,
                   y_finger_hole_radius = finger_hole_radius, x = 2, y = 1);
}
